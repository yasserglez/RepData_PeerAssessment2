...
The analysis is motivated by the following two questions:
...
If some of these packages are not available in your system, they can be 
installed by running the R command `install.packages(packages)` with the
`packages` variable defined as in the code fragment above.

The data used in the analysis was obtained from the U.S. National Oceanic and 
Atmospheric Administration's (NOAA) storm database. It is contained in a
bzip2-compressed file containing a comma-separated-value (CSV) file,
...
Additional information about how some of the variables were constructed/defined is provided
in the [Storm Data Preparation](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf)
...
a tidy dataset more suitable for the analysis in question.
...
according to the Levenshtein edit distance (plus additional processing 
to handle some conflictive cases).

Finally as part of the data processing, the `BGN_DATE` column is converted 
...
As we may see from the plot, the most harmful severe weather event to population 
health are tornadoes, as they are responsible for the greatest total number of 
both fatalities and injuries. Heat, flood and lightning are other weather events 
that cause important damages to population health. It is also interesting to note 
that there is a considerable difference in the number of injuries caused by 
tornadoes and the second event by the number of injuries, thunderstorm winds.
